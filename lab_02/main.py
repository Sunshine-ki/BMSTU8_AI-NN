...
# Обучающий набор содержит 60000 примеров, а тестовый — 10000 примеров. 
# Цифра представляется 28x28
path = 'MNIST' #os.path.join(os.path.expanduser('/path/to/MNIST-dir/'), 'MNIST')
...
def tanh(z): # Гиперболический тангенс
    return np.tanh(z)
...
if __name__ == '__main__':
    ...
    for i in range(epoch):
        ...
        #forward pass # активацинная функция при прямом ходе - гиперболический тангенс
        forwardPass = forward(X,tanhParams,tanh)
        #cost
        #cost = crossEntropyR2(y, forwardPass['A2'], lamda, tanhParams)

        #back Prop # Метод обратного распространения ошибки
    ...
    print('Accuracy:',sum(y_hat==y_test)*1/len(y_test)) # result array [7, 6, 1, ... , 4, 8, 6] 
