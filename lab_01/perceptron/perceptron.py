...
class Perceptron:
    ...
    def solve(self, inputs):
        s_result = self.s_layer.solve(inputs) # По сути просто возвращает inputs (массив длиной 56)
        # каждый нейрон А слоя имеет веса (7x8 в нашем случае)
        # Каждый нейрон перемножает попарно вес на входное значение и складывает
        # После чего применяет активационную функцию, которая смотрит на полученную сумму и
        # Если она больше или равня нулю, то возвращает 1, иначе ноль
        a_result = self.a_layer.solve(s_result) # Возвращает массив нулей и единиц (результат активационной функции над сумматором для каждого нейрона)
        # Аналогично, только результатом будет массив, содержащий -1 и 1 () (длина 29 т.к. 29 буковок мы пытаемся различить)
        return self.r_layer.solve(a_result)

    def correct(self, expected_results):
        # Корректировка весов R-слоя
